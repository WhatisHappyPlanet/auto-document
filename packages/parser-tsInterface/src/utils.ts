...
export const getIsValidPath = async(filePath:string)=>{
  const isValid = await fsPromise.access(filePath).then(()=>{
    return true;
  }).catch(()=>{
    return false;
  });
...
export const filePathAddExtension = async(filePath:string)=>{
  let flag = false,result = filePath;
...
export const handlePath = async(filePath:string,options:handlePathOptionsType)=>{
  const {currentExecPath,directoryToFilePath=false} = options;

  let absoluteFilepath = filePath;

  const parseResult = path.parse(filePath);
  absoluteFilepath = parseResult.root!=='/'?path.resolve(currentExecPath,filePath):absoluteFilepath;
  ...
  try{
    ...
    if(isDirectory && directoryToFilePath){
      absoluteFilepath = await filePathAddExtension(`${absoluteFilepath}/index`);
      return absoluteFilepath;
    }
  }catch(e){
    return Promise.reject(e);
